    ...
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          ...
          const auto& encodable_scan_response_manufacturer_data_arg = args.at(5);
          const auto* scan_response_manufacturer_data_arg = &(std::any_cast<const ManufacturerData&>(std::get<CustomEncodableValue>(encodable_scan_response_manufacturer_data_arg)));
          std::optional<FlutterError> output = api->StartAdvertising(services_arg, local_name_arg, timeout_arg, manufacturer_data_arg, add_manufacturer_data_in_scan_response_arg, scan_response_manufacturer_data_arg);
          if (output.has_value()) {
            reply(WrapError(output.value()));
            return;
          }
